...
// Fetch data efficiently using caching where appropriate
const getData = async (slug) => {
  const res = await fetch(`${process.env.WEBSIT_URL}/api/posts/${slug}`, {
    cache: "force-cache", // Cache the post data
  });
  if (!res.ok) {
    throw new Error("Failed to fetch post data");
  }
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    link: [
      ...
      { rel: "alternate", hreflang: "en", href: `${process.env.WEBSIT_URL}/${slug}` },
      { rel: "alternate", hreflang: "fr", href: `${process.env.WEBSIT_URL}/fr/${slug}` },
      { rel: "alternate", hreflang: "es", href: `${process.env.WEBSIT_URL}/es/${slug}` },
    ],
  };
}

// Optimized data fetching for popular posts
const fetchPopularPosts = async () => {
  const res = await fetch(`${process.env.WEBSIT_URL}/api/mostpopular`, {
    cache: "force-cache", // Cache the most popular posts
  });
  if (!res.ok) throw new Error("Failed to fetch popular posts");
  return res.json();
};

// const fetchDoctorData = async (slug) => {
//   const res = await fetch(`${process.env.WEBSIT_URL}/api/author/${slug}`);
//   if (!res.ok) throw new Error("Failed to fetch doctor data");
//   return res.json();
// };
...
const Page = async ({ params }) => {
  ...
  const [data, doctor, popular, comments, questions] = await Promise.all([
    getData(slug),
    fetchDoctorData(slug),
    fetchPopularPosts(),
    fetchComments(slug),
    fetchQuestions(slug),
  ]);
    ...
    <div className={styles.container}>
      <section className={styles.leftSidebar}>
        {doctor && (
          <div className={styles.doctor}>
            <img src={doctor?.coverImage} width={500} height={300} alt="cover" priority />
            <div className={styles.sidebarProfile}>
              <img src={doctor?.image} alt="user" width={100} height={100} />
              <h3>{doctor?.name}</h3>
              <h4>{doctor?.specialist}</h4>
              <p className={styles.degree}>{doctor?.degree}</p>
      ...
      <section className={styles.mainContent}>
        ...
        <article className={styles.article}>
          ...
          <h1>{data?.title}</h1>
          <div className={styles.articleBody} dangerouslySetInnerHTML={{ __html: data?.desc }} />
        </article>
        ...
        <div className={styles.postLinks}>
          ...
          <RatingComponent initialRating={data?.totalRating?.average || 4.5} postId={data.id} />
          <QuestionAndAnswer length={questions.length} />
          <ShareButton title={data?.title} url={`${process.env.WEBSIT_URL}/${slug}`} />
        </div>
